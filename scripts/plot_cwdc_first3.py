#!/usr/bin/env python3
"""
Plot the first three Y_cwdc columns on a global map.

Default behavior: plot BOTH prediction and ground-truth maps.

Usage (both by default):
  python scripts/plot_cwdc_first3.py \
    [--pred-csv ./cnp_inference_entire_dataset/cnp_predictions/soil_2d_predictions/predictions_Y_cwdc_vr.csv] \
    [--gt-csv ./cnp_inference_entire_dataset/cnp_predictions/soil_2d_ground_truth/ground_truth_Y_cwdc_vr.csv] \
    [--output-dir ./cnp_inference_entire_dataset/cnp_predictions/ai_model_comparison_plots/] \
    [--cartopy]

Single-file mode:
  python scripts/plot_cwdc_first3.py --only pred  # or --only gt

Assumes CSV columns are:
  Longitude, Latitude, <value_col_1>, <value_col_2>, <value_col_3>, ...

The script selects the first three value columns after Longitude and Latitude.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot first 3 Y_cwdc columns globally")
    parser.add_argument("--pred-csv", default="./cnp_inference_entire_dataset/cnp_predictions/soil_2d_predictions/predictions_Y_cwdc_vr.csv", help="Path to predictions CSV")
    parser.add_argument("--gt-csv", default="./cnp_inference_entire_dataset/cnp_predictions/soil_2d_ground_truth/ground_truth_Y_cwdc_vr.csv", help="Path to ground-truth CSV")
...
def wrap_longitudes(longitudes: np.ndarray) -> np.ndarray:
    ...
    # Where lon > 180, subtract 360
    lon = np.where(lon > 180.0, lon - 360.0, lon)
    return lon


def pick_value_columns(header: List[str]) -> List[str]:
    """Return the first three value columns after Longitude, Latitude.

    We assume first two columns are named 'Longitude' and 'Latitude'. If names differ by case,
    we'll try to locate them case-insensitively.
    """
    # Normalize header for locating lon/lat
    ...
    if len(value_cols) < 3:
        raise ValueError("CSV does not have at least 3 value columns after coordinates")
    return value_cols
...
def plot_with_cartopy(ax, lon, lat, values, title: str, marker_size: float, vmin: float, vmax: float):
    import cartopy.crs as ccrs  # Imported lazily to avoid hard dependency
...
def generate_map_for_csv(csv_path: str, output_path: str, args: argparse.Namespace) -> None:
...
def main() -> int:
    ...
    pred_out = os.path.join(args.output_dir, "Y_cwdc_first3_global.png")
    gt_out = os.path.join(args.output_dir, "Y_cwdc_GT_first3_global.png")

    try:
        if args.only == "pred":
            generate_map_for_csv(args.pred_csv, pred_out, args)
        elif args.only == "gt":
            generate_map_for_csv(args.gt_csv, gt_out, args)
        else:
