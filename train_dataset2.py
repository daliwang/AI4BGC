#!/usr/bin/env python3
"""
Training script for Dataset 2 (1_trendy_case) only.
"""
...
from config.training_config import get_dataset2_config
...
def main():
    """Main training function for Dataset 2."""
    # Create output directory
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = f"results/dataset2_trendy_case_{timestamp}"
    ...
    logger.info("Starting training with configuration: dataset2_trendy_case")
    
    try:
        # Get configuration
        config = get_dataset2_config()
